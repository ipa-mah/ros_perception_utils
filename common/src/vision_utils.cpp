...
bool VisionUtils::readOBJFile(const std::string file_name,TriMesh& mesh)
{
...
bool VisionUtils::readPLYFile(const std::string file_name, TriMesh& mesh)
{
    ...
    {
        mesh.vertices[i].pos = glm::vec3(cloud.points[i].x,cloud.points[i].y,cloud.points[i].z);
        mesh.vertices[i].normal = glm::vec3(cloud.points[i].normal_x,cloud.points[i].normal_y,cloud.points[i].normal_z);
    }
