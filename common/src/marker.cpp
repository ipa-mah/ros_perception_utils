...
Marker::Marker(int markers_x,int markers_y,float marker_length,int dictionary_id,int marker_separation, int border_bits,int margins )
{
    dist_coeffs_ = cv::Mat();
    camera_matrix_ = cv::Mat();
    aruco_params_.markers_x = markers_x;
    aruco_params_.markers_x = markers_x;
    aruco_params_.border_bits = border_bits;
    aruco_params_.dictionary_id = dictionary_id;
    aruco_params_.marker_length = marker_length;
    aruco_params_.margins = margins;
    aruco_params_.marker_separation = marker_separation;
}
...
unsigned long Marker::createChaRucoMarker()
{
    ...
    charuco_board_ = cv::aruco::CharucoBoard ::create(charuco_params_.square_x,charuco_params_.square_y,
                                                      (double)charuco_params_.square_length,(double)charuco_params_.marker_length,dictionary_);
...
unsigned long Marker::createArucoMarker(cv::Mat& board_images)
{
    ...
    board = cv::aruco::GridBoard::create(aruco_params_.markers_x, aruco_params_.markers_y,
                                         double(aruco_params_.marker_length),double(aruco_params_.marker_separation), dictionary);
    //Draw it on image
...
bool Marker::estimatePoseCharuco(const cv::Mat &frame,Eigen::Matrix4d& marker_to_camera )
{
    ...
    cv::aruco::refineDetectedMarkers(frame_clone, board_, markerCorners, markerIds, rejectedMarkers,
    ...
    if(markerIds.size() > 0)
        interpolatedCorners =cv::aruco::interpolateCornersCharuco(markerCorners, markerIds, frame_clone, charuco_board_,
                                                                  charucoCorners, charucoIds, camera_matrix_, dist_coeffs_);

    // estimate charuco board pose
    bool validPose = false;
    if(camera_matrix_.total() != 0)
        validPose = cv::aruco::estimatePoseCharucoBoard(charucoCorners, charucoIds, charuco_board_,
                                                        camera_matrix_, dist_coeffs_, pose.rot, pose.trans);
...
bool Marker::estimatePoseAruco(const cv::Mat& frame, Eigen::Matrix4d& camera_to_marker)
{
